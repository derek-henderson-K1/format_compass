...
class update_course extends external_api {
            ...
            [
                ...
                'section' => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            'cutoff' => new external_multiple_structure(
                                new external_single_structure(
                                    array(
                                        'assessment' => new external_value(
                                            PARAM_TEXT,
                                            'The new assessments cutoff position.'
                                        )
                                    )
                                )
                            )
    ...
    public static function execute(
        string $action, int $courseid, array $ids = [],
            ?int $targetsectionid = null, ?int $targetcmid = null, $section): string {
        global $CFG;
        ...
        // Get the assessments cutoff and save it.
        $assessmentscutoff = $params['section'][0]['cutoff'][0]['assessment'];
        $courseformat->update_course_format_options(
            ['id' => $courseid, 'assessmentscutoff' => $assessmentscutoff]
        );
        ...
        // Add course_format_options state updates
