 ...
 * Copied / Adapted from course/editsection.php
 *
...
$tabreturn = optional_param('tab', '', PARAM_RAW);

$PAGE->set_url('/course/format/compass/editsection.php', array('id'=>$id, 'sr'=> $sectionreturn));
...
if ($deletesection) {
    ...
    if (course_can_delete_section($course, $sectioninfo)) {
        ...
        if ($confirm) {
            ...
            $deleteurl = new moodle_url('/course/format/compass/editsection.php', $optionsyes);
...
require_once($CFG->dirroot. '/course/format/compass/lib.php');
...
if ($mform->is_cancelled()){
    ...
    if ($sectionreturn == 0) {
        if ($tabreturn !== '') {
            $urlparams['tab'] = $tabreturn;
        }
    } else {
        $urlparams['section'] = $sectionreturn;
    }
    $newurl = new moodle_url('/course/view.php', $urlparams);
    redirect($newurl);
